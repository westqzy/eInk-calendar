...
CurrentWeather QWeather::GetCurrentWeather(String location)
{
    ...
    String requestUrl = ((_apiVersion == APIVersion::DEV) ? QWEATHER_WEATHERAPI_BASE_URL_DEV : QWEATHER_WEATHERAPI_BASE_URL_COMMERCIAL) + "/now?" + "location=" + location + "&key=" + _key + "&lang=" + _lang + "&unit=" + ((_unitType == UnitType::METRIC) ? "m" : "i");
    ...
    if (httpClient.begin(requestUrl))
    {
        ...
        if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY)
        {
            ...
            // 调用解压函数
            int rr=ArduinoUZlib::decompress(inbuff, size, outbuf,outsize);
            // 输出解密后的数据到控制台。
            Serial.println("aaaaaaaaaaaaaaaaaaaaaaaaa");
            String payload(outbuf, outsize);
            Serial.println(payload);
...
vector<DailyWeather> QWeather::GetDailyWeather(String location, DailyPredictionType dailyPredictionType)
{
    ...
    String requestUrl = ((_apiVersion == APIVersion::DEV) ? QWEATHER_WEATHERAPI_BASE_URL_DEV : QWEATHER_WEATHERAPI_BASE_URL_COMMERCIAL) + "/"+ String(dailyPredictionType) +"d?" + "location=" + location + "&key=" + _key + "&lang=" + _lang + "&unit=" + ((_unitType == UnitType::METRIC) ? "m" : "i");
    Serial.println(requestUrl);
    if (httpClient.begin(requestUrl))
    {
        u8_t httpCode = httpClient.GET();
...
CurrentAirQuality QWeather::GetCurrentAirQuality(String location)
{
    ...
    String requestUrl = ((_apiVersion == APIVersion::DEV) ? QWEATHER_AIRAPI_BASE_URL_DEV : QWEATHER_AIRAPI_BASE_URL_COMMERCIAL) + "/now?" +  "location=" + location + "&key=" + _key + "&lang=" + _lang;
    Serial.println(requestUrl);
    if (httpClient.begin(requestUrl))
    {
        u8_t httpCode = httpClient.GET();
